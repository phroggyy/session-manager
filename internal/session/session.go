...
type Session struct {
	state    *State
	stateDir string // ~/.sm/sessions/<repo-hash>/
	mu       sync.RWMutex
}
...
// NewSession creates a new session for the given repository path.
func NewSession(repoPath string) (*Session, error) {
	...
	sessionDir := GetSessionDir(absPath)
...
// LoadSession loads an existing session from disk for the given repository path.
func LoadSession(repoPath string) (*Session, error) {
	absPath, err := filepath.Abs(repoPath)
	if err != nil {
		return nil, fmt.Errorf("failed to get absolute path: %w", err)
	}
